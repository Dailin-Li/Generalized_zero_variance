...
model {
    ...
    for(i in 1:n) {
        label[i] ~ bernoulli(inv_logit(coef1*length[i] + coef2*widthLeft[i] + coef3*rightEdge[i] + coef4*bottomMargin[i] + coef5));
  }
}
...
A_mat=construct_A_mat(mcmc_samples,grad_log_prob_val)
#%%
cv_sample_1=construct_cv_sample(A_mat,mcmc_samples,mode=1)
cv_sample_2=construct_cv_sample(A_mat,mcmc_samples,mode=2)
cv_sample_3=construct_cv_sample(A_mat,mcmc_samples,mode=3)
#%%
import matplotlib.pylab as plt
fig_size=(8, 8)
num_samples = mcmc_samples.shape[0]
